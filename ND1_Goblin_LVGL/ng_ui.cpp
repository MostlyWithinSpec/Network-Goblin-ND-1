...
/* ------------------------------------------------------------
 *  Network screen: Ping/Trace buttons (Gateway + Google).
 *  Autotest screen: cable -> if good -> ping GW + Google, PASS/FAIL.
 * ============================================================ */
#include "ng_config.h"
#include "ng_ui.h"
#include "ng_eth.h"
...
static lv_obj_t *scr_home,*scr_cable,*scr_net,*scr_auto;
static int cur=0;
...
static void ev_theme(lv_event_t* e){
  ...
  buildAll(); lv_screen_load(cur==0?scr_home:cur==1?scr_cable:cur==2?scr_net:scr_auto);
}
...
static void buildAll(){ buildHome(); buildCable(); buildNet(); buildAuto(); }
