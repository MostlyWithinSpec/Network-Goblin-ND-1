/* ng_ui.cpp - HAND-CODED LVGL screens (Home / Cable / Network / Autotest)
 * ------------------------------------------------------------
 *  THEME-CRASH FIX: the dark-mode switch no longer rebuilds screens
 *  from inside its own event (which orphaned the live switch and
 *  caused a StoreProhibited panic). It now sets a flag; the rebuild
 *  happens safely from ngUiService(), and old screens are deleted
...
static lv_obj_t *scr_home=NULL,*scr_cable=NULL,*scr_net=NULL,*scr_auto=NULL;
static int cur=0;
...
static void ev_theme(lv_event_t* e){
  // ONLY set state + a flag here. Do NOT rebuild screens inside the
  // event of a widget that lives on a screen we're about to delete.
  g_dark = lv_obj_has_state((lv_obj_t*)lv_event_get_target(e), LV_STATE_CHECKED);
  want_theme_rebuild = true;
}
...
static void navBtn(lv_obj_t* parent,const char* title,const char* sub,lv_event_cb_t cb){
  ...
  lv_obj_set_width(b,lv_pct(100)); lv_obj_set_height(b,76);
  lv_obj_set_style_bg_color(b,col_surf(),0);
  lv_obj_set_style_border_color(b,col_border(),0);
...
static void buildHome(){
  ...
  navBtn(b,"TOOLS","Settings / Logs / About",ev_goHome);
}
...
static void buildAll(){
  // Delete previous screens first: prevents memory leaks AND dangling
  // references (which caused the dark-mode StoreProhibited panic).
  ...
  buildHome(); buildCable(); buildNet(); buildAuto();
}
...
void ngUiService(){
  // ---- deferred theme rebuild (fixes the dark-mode crash) ----
  if(want_theme_rebuild){
    want_theme_rebuild = false;
    int keep = cur;
    buildAll();
    lv_screen_load(keep==0?scr_home:keep==1?scr_cable:keep==2?scr_net:scr_auto);
  }
